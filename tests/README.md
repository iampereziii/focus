...
**Status 2026-09-07:** 277 tests, 17 files, all passing. `npm run build`,
...
| `compact-density.test.ts` | The compact pass changes how facts feel, never which are present: a census proving `compact:hidden` occurs **exactly once** (the WHY, shipped with its reveal), the `(pointer: coarse)` 40 px touch floor on every pressable primitive, **no `fixed bottom-*` control anywhere** (the FAB is in the nav now, and ⌘K stays bound from every page), `SegmentedControl`'s `1fr` columns at all five call sites, and ADR-0003's budget intact — seven targets, `Filler` and `I drifted` each full-width on their own row, no decorative non-target cell. |

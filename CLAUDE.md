...
- **Size responds to height; `compact:` carries layout.** That split is the whole design system and both halves are lint- and test-enforced:
  - **The fluid ramp** — four type ROLES and a rhythm scale in `globals.css`, all `clamp()`ed on `vh`: `text-label` (12→14 px, labels/meta/hints) · `text-body` (13→15, body, rows, controls) · `text-title` (15→19, screen and session titles) · `text-clock` (20→27). Spacing: `rhythm` / `rhythm-tight` / `rhythm-loose` / `gutter` / `control-y` / `writing` / `grid-row`. **Every floor is the size that shipped before the ramp went fluid**, so the 337 px worst case renders what is already proven to fit and everything above it grows. **Nothing below 12 px, and a fifth type role is a regression** — `compact-density.test.ts` asserts the four roles, the ≥12 px `clamp()` minimum, and that no fixed `text-sm`/`text-[…rem]` survives anywhere.

...
- **The window is measured, not guessed: 500 px wide (a HARD FLOOR, never narrower) × 337–815 px tall, resized arbitrarily.** Three earlier passes each calibrated against a different guess (340×500, then 500×375–485, then 480×430) and each was wrong in a way the next had to undo. Design against these numbers; if they change, re-measure rather than re-guess.
- **Size responds to height; `compact:` carries layout.** That split is the whole design system and both halves are lint- and test-enforced:
  - **The fluid ramp** — four type ROLES and a rhythm scale in `globals.css`, all `clamp()`ed on `vh`: `text-label` (14 px, effectively fixed within the window) · `text-body` (15, effectively fixed) · `text-title` (18→19, screen and session titles) · `text-clock` (23→27). Spacing: `rhythm` / `rhythm-tight` / `rhythm-loose` / `gutter` / `control-y` / `writing` / `grid-row`. **Floors were raised twice on 2026-09-09** (feature-brief-larger-type-floors.md — the second raise calibrated against 105% browser zoom on the first) **and the growth formulas were deliberately left alone both times**, so each role now renders its floor as one fixed size until well past 500 px of window height and only grows above that. **The 500×337 hand check this brief requires has not been re-run against the current numbers** — do that before trusting them at the tight end; don't infer it from the earlier pass, which measured a smaller size. **Nothing below 12 px, and a fifth type role is a regression** — `compact-density.test.ts` asserts the four roles, the ≥12 px absolute `clamp()` minimum, each role's raised floor, the unchanged growth formula, and that no fixed `text-sm`/`text-[…rem]` survives anywhere.
  - **The `compact` variant**, declared once (`max-width: 520px`, `max-height: 800px`) — never a per-component media query or a `sm:`/`md:` breakpoint. It may only change *layout* (flex direction, wrapping, a rule, the one reveal). **A `compact:text-*` or a `compact:` numeric spacing step is a second ramp and fails the test**; keyword values (`p-0`, `max-w-none`, `min-h-full`, `min-w-0`) are layout and are fine. Neither arm may be removed — 800 px is the measured number from the short-viewport brief, and at 500 px wide the width arm is what keeps `compact` on at all.
  - **`compact:` may only change how a fact looks, never whether it is there:** `compact:hidden` occurs exactly once in the whole tree (the WHY on the session screen, shipped with its reveal) and `compact-density.test.ts` counts it.

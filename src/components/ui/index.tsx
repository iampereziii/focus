...
export function Button({
  ...
  return (
    <button
      ...
      className={`tap inline-flex items-center justify-center gap-2 rounded-lg px-4 py-control-y text-body font-medium transition compact:rounded-md ${
      ...
      {options.map((opt, i) => {
        ...
        return (
          <button
            ...
            className={`inline-flex min-w-0 items-center justify-center gap-1.5 px-2 py-control-y text-body font-medium outline-none transition focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-foreground ${
...
export function Input({
  ...
  return (
    <input
      {...props}
      className={`w-full rounded-lg border border-neutral-300 bg-transparent px-3 py-control-y text-body outline-none focus:border-neutral-900 compact:rounded-md dark:border-neutral-700 dark:focus:border-neutral-300 ${className}`}
    />
  );
}
...
export function Field({
  ...
  return (
    <label className={`block ${className}`}>
      ...
      {hint !== undefined && (
        <span className="ml-2 text-label opacity-60">{hint}</span>
      )}
      <div className="mt-rhythm-tight">{children}</div>
      {error != null && error !== "" && (
        <p className="mt-rhythm-tight text-label text-red-600 dark:text-red-400">
          {error}
        </p>
      )}
    </label>
...
export function Badge({ variant }: { variant: BadgeVariant }) {
  return (
    <span
      className={`inline-flex shrink-0 items-center rounded px-1.5 py-0.5 text-label font-medium ${BADGE_VARIANTS[variant]}`}
    >
      {variant}
    </span>
  );
...
export function Sheet({
  ...
  return (
    ...
    >
      <div
        className="focus-sheet-in w-full max-w-lg rounded-xl border border-border bg-surface p-gutter shadow-xl compact:flex compact:min-h-full compact:max-w-none compact:flex-col compact:rounded-none compact:border-0 compact:shadow-none"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="mb-rhythm text-label font-semibold uppercase tracking-wider text-muted">
          {title}
        </h2>
        {children}
      </div>
